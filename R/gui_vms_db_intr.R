        ...
        if(nrow(vessel) == 0)
        {
          ...
          for(ind in 1:maxtrk_n)
          {
            ...
            if(numrow > 4)
            {
              ...
              COO_UTM <- data.frame("X" = numeric(nrow(COO_LL)),
                                    "Y" = numeric(nrow(COO_LL)),
                                    "zone" = numeric(nrow(COO_LL)))
              
              COO_UTM <- suppressMessages(convUL(COO_LL))
              ...
              #           hp["LON"] <- tracki["SPE"] * sin(tracki["HEA"]*(2*pi)/360)
              #           hp["LAT"] <- tracki["SPE"] * cos(tracki["HEA"]*(2*pi)/360)
              ...
              attr(utm, "zone") <- attr(COO_UTM, "zone")
              
              ll <- suppressMessages(convUL(utm))
